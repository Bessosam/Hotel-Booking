...
function AddHotel() {
    ...
    const [inputData, setInputData] = useState({
        roomType: '',
        pricePerNight: '',
        amenities: { // fixed spelling from "ameninties" to "amenities"
            "Free Wi-Fi": false,
    ...
    return (
        <form className='px-4 mt-4 pt-4 '>
            ...
            <div className='w-full flex sm:flex-col sm:gap-4 mt-4'>
                ...
                <p className='text-gray-600 font-bold mt-4'>Amenities</p>

                <div className='flex flex-col flex-wrap text-gray-400 mt-2 '>
                    {Object.keys(inputData.amenities).map((amenity, index) => (
                        <div key={index}>
                            <label htmlFor={`amenities${index + 1}`} className='flex items-center gap-2'>
                                <input
                                    type='checkbox'
